...
export const updatePassword = asyncHandler(async (req, res, next) => {
    ...
    const match = await compareHash({ plaintext: oldPassword, hashValue: req.user.password })
    if (!match) {
        return next(new Error("Invalid old password", { cause: 404 }))
    }

    if(req.user.oldPasswords?.length){
        for ( const hashedPassword of req.user.oldPasswords){
            if (await compareHash({ plaintext:password, hashValue: hashedPassword })){
                        return next(new Error("This password has been used before", { cause: 404 }))
            }
        }
    }
    let updatedData={}
